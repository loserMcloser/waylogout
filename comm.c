...
ssize_t read_comm_request(char **buf_ptr) {
	...
	if (amt == 0) {
		...
		swaylogout_log_errno(LOG_ERROR, "read pw request");
		return -1;
	}
	swaylogout_log(LOG_DEBUG, "received pw check request");
	char *buf = malloc(size);
	if (!buf) {
		swaylogout_log_errno(LOG_ERROR, "failed to malloc pw buffer");
		return -1;
	}
	size_t offs = 0;
	do {
		...
		if (amt <= 0) {
			swaylogout_log_errno(LOG_ERROR, "failed to read pw");
			return -1;
		}
		offs += (size_t)amt;
	} while (offs < size);
...
bool write_comm_reply(bool success) {
	if (write(comm[1][1], &success, sizeof(success)) != sizeof(success)) {
		swaylogout_log_errno(LOG_ERROR, "failed to write pw check result");
		return false;
	}
	return true;
}
...
bool spawn_comm_child(void) {
	if (pipe(comm[0]) != 0) {
		swaylogout_log_errno(LOG_ERROR, "failed to create pipe");
		return false;
	}
	if (pipe(comm[1]) != 0) {
		swaylogout_log_errno(LOG_ERROR, "failed to create pipe");
	...
	pid_t child = fork();
	if (child < 0) {
		swaylogout_log_errno(LOG_ERROR, "failed to fork");
		return false;
	} else if (child == 0) {
		close(comm[0][1]);
		close(comm[1][0]);
...
bool write_comm_request(struct swaylogout_password *pw) {
	...
	if (write(comm[0][1], &len, sizeof(len)) < 0) {
		swaylogout_log_errno(LOG_ERROR, "Failed to request pw check");
		goto out;
	}
	...
	do {
		...
		if (amt < 0) {
			swaylogout_log_errno(LOG_ERROR, "Failed to write pw buffer");
			goto out;
		}
		offs += amt;
	} while (offs < len);
...
bool read_comm_reply(void) {
	...
	if (read(comm[1][0], &result, sizeof(result)) != sizeof(result)) {
		swaylogout_log_errno(LOG_ERROR, "Failed to read pw result");
		result = false;
	}
	return result;
}
